  ...
  return (
    <div className="relative w-full max-w-2xl mx-auto mb-12">
      <div className="flex items-center border-4 border-ash bg-steel/10 focus-within:border-signal transition-colors group">
        ...
        <input
          type="text"
          placeholder="SEARCH ARCHIVE // AXIOMS // TESTIMONY"
      ...
      {query && filteredItems.length > 0 && (
        <div className="absolute top-full left-0 right-0 z-[100] mt-1 bg-concrete border-4 border-ash shadow-[12px_12px_0_rgba(0,0,0,0.8)]">
          {filteredItems.map(item => (
            <Link 
              key={`${item.type}-${item.slug}`}
              href={item.type === 'node' ? `/wiki/node/${item.slug}` : `/wiki/essays/${item.slug}`}
              onClick={() => setQuery('')}
