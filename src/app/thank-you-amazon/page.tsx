...
function ThankYouAmazonContent() {
  const searchParams = useSearchParams();
  const vol = searchParams ? searchParams.get('vol') : '1';

  const associateTag = "materialistch-20";
  const vol1Asin = "B0FMN5PDZ4";
  const vol2Asin = "B0HCM882QP";

  const targetAsin = vol === '2' ? vol2Asin : vol1Asin;
  const volumeTitle = vol === '2' ? "Volume II: Historical Sediments & Applied Mechanics" : "Volume I: Moral Motion & Mechanics";

  const amazonUrl = `https://www.amazon.com/dp/${targetAsin}?tag=${associateTag}&maas=maas_mc_google_ads_conversion&ref_=mc_google_ads`;
  ...
  return (
    <div className="max-w-2xl bg-slate p-8 md:p-12 border border-amber/30 rounded-lg shadow-2xl">
      ...
      <h2 className="text-lg font-mono text-amber mb-6">
        {volumeTitle}
      </h2>
      ...
      <div className="flex flex-col items-center gap-4">
        ...
        <div className="flex gap-4 mt-6">
          <a href={`https://www.amazon.com/dp/${vol1Asin}?tag=${associateTag}`} target="_blank" rel="noopener noreferrer" className="text-xs font-mono text-amber hover:underline">
            View Volume I on Amazon
          </a>
          <span className="text-muted">|</span>
          <a href={`https://www.amazon.com/dp/${vol2Asin}?tag=${associateTag}`} target="_blank" rel="noopener noreferrer" className="text-xs font-mono text-amber hover:underline">
            View Volume II on Amazon
          </a>
        </div>
...
export default function ThankYouAmazonPage() {
  return (
    <main className="min-h-screen bg-concrete text-ash selection:bg-signal selection:text-concrete flex flex-col justify-center items-center px-6 py-24 text-center">
      <Suspense fallback={<div className="text-amber font-mono">Loading Amazon Transfer...</div>}>
        <ThankYouAmazonContent />
      </Suspense>
    </main>
  );
}
